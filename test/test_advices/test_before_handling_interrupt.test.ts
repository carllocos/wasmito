...
describe('Test before handling Interrupts', function () {
  let wasm: WasmModule;
  ...
  it('before handling interrupt advice should read topic and payload', async () => {
    let interrupt: undefined | WASM.Event;
    analysis.beforeHandlingInterrupt((i: ReadOnlyInterrupt) => {
      interrupt = i;
    });
  ...
  it('multiple advices before handling interrupt should run sequentially', async () => {
    const orderCalled: number[] = [];
    analysis.beforeHandlingInterrupt(() => orderCalled.push(1));
    analysis.beforeHandlingInterrupt(() => orderCalled.push(2));
    analysis.beforeHandlingInterrupt(() => orderCalled.push(3));
  ...
  it('before handling interrupt mutability advice should allow edit topic and payload', async () => {
    let topic1 = '';
    let payload1 = 'nothing';
    let topic2 = '';
    let payload2 = 'nothing';
    analysis.beforeHandlingInterruptMut((i: WritableInterrupt) => {
      topic1 = i.topic;
      payload1 = i.payload;
      i.topic = 'new topic';
