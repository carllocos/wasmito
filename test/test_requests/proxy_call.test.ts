...
describe('Intergation Test: Proxy Calls to a Mocked Target VM produces the right payloads', () => {
  it('Proxy Call a primitive function with 3 arguments', async () => {
    ...
    const targetVM = new MockVM(platform);

    const sourceCodeCompilationArgs: WATCompilerArgs = {
      sourceCodePath: './test/data/test-example-proxy-call.wat',
    };
    const uploaded = await targetVM.uploadSourceCode(sourceCodeCompilationArgs);
