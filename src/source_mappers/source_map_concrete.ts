...
export class SourceMapConcrete extends SourceMap {
  ...
  constructor(
    ...
    super(absolutePathSources, wasmPath);
  ...
  // TODO fix make that row and col start at 0 make the users adapt and vice versa too
  ...
  async buildComplemtaryContext(): Promise<void> {
    const parser = await createRustParser();
  ...
  static async fromSourceMapPath(
    ...
    const sm = new SourceMapConcrete(
      pathToSourceMap,
      wasmPath,
      sourcesAbsPath,
      sources,
