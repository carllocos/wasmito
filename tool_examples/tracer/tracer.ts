...
import { DefaultSourceOffsetStart, ReadSourceSpec, SourceMapConfig } from '../../src/source_mappers/source_map_builder';
...
async function registerHooks(
    ...
    for (const n of em.languageAdaptor.sourceCFGs.allNodes()) {
        ...
        inspectStack.onSubscriptionData = writer.write(
            n,
        );
...
async function run(): Promise<void> {
    ...
    const srcFileMapper = new Map<string, string>([['blink/blink.ts', sourcePath]]);
    ...
    const compilationArgs: WasmCompilerArgs = {
        wasmPath: wasmApp,
        mappingsJSON: await ReadSourceSpec(debugInfo, wasmApp, DefaultSourceOffsetStart, sourceMapConfig)
    };
