...
class WriteCSV {
  ...
  public write(row: string[]): void {
    if (row.length !== this.header.length) {
      throw new Error(
        `Row length ${row.length} does not match header length ${this.header.length}`,
      );
    }
    if (this.firstWrite) {
...
export class StoreTrace {
  ...
  constructor(output: string, bufferSize: number) {
    this.writer = new WriteCSV(output, bufferSize, [
      'source',
      'linenr',
      'colnr',
      'addr',
  ...
  private writeBefore(n: SourceCFGNode, state: WasmState): void {
