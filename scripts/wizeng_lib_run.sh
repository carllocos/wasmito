#!/bin/sh
#
# Usage: wizeng_lib_run.sh <x86-64|jvm> <wasm-module-or-dir> <output-dir> [optimise|no-opt] <num-runs> [timeout-seconds] [whamm-module] [libs]
#
# Same as wizeng_run.sh, except that the whamm module(s) may depend on user
# libraries (e.g. `use cache;`), which are also loaded into wizeng.
#
...
#
# [libs] is a comma-delimited list of user libraries, each either a bare path
# to a lib's .wasm or <lib_name>=<lib_path> (the same format given to
# wei_compile_libs.sh / whamm's --user-libs), e.g.:
#   cache=tests/libs/cache/cache.wasm
# Each lib is appended to wizeng's --monitors list after whamm_core.wasm. The
# module name wizeng links a lib under is its file's basename, so it must
# match the lib name the whamm module imports (cache.wasm -> "cache").
# [libs] is only used when [whamm-module] is provided, and applies to every
# whamm module if that is a directory.
#
...
if [ -z "$TARGET" ] || [ -z "$MODULE_ARG" ] || [ -z "$OUT_DIR" ] || [ -z "$NUM_RUNS" ]; then
    echo "Usage: $0 <x86-64|jvm> <wasm-module-or-dir> <output-dir> [optimise|no-opt] <num-runs> [timeout-seconds] [whamm-module] [libs]" >&2
    exit 1
fi
...
# Turn "name=path,path2" into "+path+path2" for wizeng's --monitors list.
LIBS_MONITORS=""
if [ -n "$LIBS_ARG" ]; then
    OLD_IFS=$IFS
    IFS=,
    for LIB in $LIBS_ARG; do
        LIB_PATH=${LIB#*=}
        if [ ! -f "$LIB_PATH" ]; then
            echo "error: lib '$LIB_PATH' does not exist" >&2
            exit 1
        fi
        LIBS_MONITORS="$LIBS_MONITORS+$LIB_PATH"
    done
    IFS=$OLD_IFS
fi
